...
namespace EfQueryableExtensions
{
  public static class EfQueryableExtensions
  {
    ...
    ) where T : class
    {
      ...
      var pagedQuery = Paginate(baseQuery, size.Value, (page.Value - 1) * size.Value);
      var pagedFilteredAndIncludedQuery = FilterAndInclude(pagedQuery, filter, includes);
      int countTotal() => FilterAndInclude(baseQuery, filter, includes).Count();
    ...
    {
      var (pagedQuery, countTotal) = QueryPage(baseQuery, page, size, filter, includes);
    ...
    ) where T : class
    {
      ...
      var (sortedPagedFilteredAndIncludedQuery, countTotal) = QueryPage(sortedQuery, page, size, filter, includes);
    ...
    private static IQueryable<T> Sort<T>(IQueryable<T> baseQuery, KeyValuePair<Expression<Func<T, object>>, SortOrder>[] orderBys)
    {
    ...
    private static IQueryable<T> Include<T>(IQueryable<T> baseQuery, Func<IQueryable<T>, IIncludableQueryable<T, object>> includes) =>
